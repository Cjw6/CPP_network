...
template <typename... Args> class SignalSlot {
...
public:
  ...
  SignalSlot(SignalSlot const & /*unused*/) {}

  SignalSlot &operator=(SignalSlot const &other) {
    if (this != &other) {
      disconnect_all();
    }
    return *this;
  ...
  SignalSlot(SignalSlot &&other) noexcept
      : _slots(std::move(other._slots)), _current_id(other._current_id) {}

  SignalSlot &operator=(SignalSlot &&other) noexcept {
    if (this != &other) {
